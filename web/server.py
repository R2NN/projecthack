...
DEFAULT_CHECKPOINT = Path(
    os.environ.get(
        ...
        os.environ.get(
            "DETECTOR_CHECKPOINT",
            str(ARTIFACTS_ROOT / "models" / "rfdetr_small_price_tag_all_annotated_tiled1280_e8_checkpoint_best_total.pth"),
        ),
    )
)
...
DEFAULT_CATALOG = Path(
    os.environ.get("LENTA_CATALOG_PATH", os.environ.get("CATALOG_PATH", str(ARTIFACTS_ROOT / "data" / "db_hack.csv")))
)
SAMPLE_CSV = ARTIFACTS_ROOT / "data" / "sample.csv"
...
def job_csv_path(job: dict[str, object], manifest: dict[str, object] | None = None, run_root: Path | None = None) -> Path:
    direct = Path(str(job.get("csv_path", "")))
    if str(direct) and direct.exists():
        return direct
    manifest = manifest if manifest is not None else job_manifest(job)
    videos = manifest.get("videos", []) if manifest else []
    if isinstance(videos, list) and videos and isinstance(videos[0], dict):
        final_csv = Path(str(videos[0].get("final_csv", "")))
        if str(final_csv) and final_csv.exists():
            return final_csv
    if run_root:
...
def job_metrics_payload(job_id: str) -> dict[str, object] | None:
    ...
    review_html = Path(str(job.get("review_html_path", "")))
    if not review_html.exists():
        review_html = job_root / "review" / "review_report.html"
