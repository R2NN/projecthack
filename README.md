...
Веса не коммитятся в Git. Это осознанное решение:

- checkpoint весит около 121 МБ;
- модель является производным артефактом обучения;
- правила хакатона и права на данные могут ограничивать публичную публикацию весов;
- GitHub-история должна оставаться лёгкой и чистой.

Для судей самый удобный вариант — приложить checkpoint отдельным артефактом к релизу GitHub или передать архивом вместе с `db_hack.csv`. Репозиторий уже ожидает файл по пути:

```text
artifacts/models/rfdetr_small_price_tag_all_annotated_tiled1280_e8_checkpoint_best_total.pth
```

`db_hack.csv` тоже не включён в Git, потому что это каталог из условий задачи. Если у судей есть исходные материалы хакатона, достаточно положить каталог в `artifacts/db_hack.csv`.
...
Репозиторий очищен от тяжёлых и потенциально закрытых файлов. В GitHub не включены:

- видео с полок;
- `db_hack.csv`;
- обучающие датасеты;
- веса моделей (`.pth`, `.pt`, `.ckpt`, `.onnx`);
...
- закрытые данные заказчика не попадают в публичную историю Git;
- веса модели распространяются отдельно и могут быть заменены новым checkpoint без переписывания репозитория;
...
- публичная часть поставки воспроизводима без публикации коммерческих данных.
...
`check_artifacts.py` завершится ошибкой на чистом clone, если артефакты ещё не положены в `artifacts/`. Это ожидаемо.
