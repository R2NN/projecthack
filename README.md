# Полка под контролем (Shelf Vision): распознавание ценников по видео полки

«Полка под контролем» (Shelf Vision) принимает MP4-видео прохода робота вдоль стеллажа и возвращает CSV в формате `sample.csv`: название товара, цены, скидки, barcode/sku, координаты ценника, QR-поля и служебные признаки. Решение рассчитано на поток видео из разных магазинов: результаты сохраняются в SQLite, crops для проверки уходят в отдельный HTML/ZIP-отчёт, а неуверенные предсказания складываются для последующего дообучения.
...
├── docs/media/          # место для скриншота и видео демонстрации
